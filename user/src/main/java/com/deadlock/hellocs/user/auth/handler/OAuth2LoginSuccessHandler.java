package com.deadlock.hellocs.user.auth.handler;
...
public class OAuth2LoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        CookieUtils.addRefreshTokenCookie(response, refreshToken, jwtTokenProvider.getRefreshValidity());

        LoginUserData userData = isUser ? null : extractUserData(oAuth2User);

        response.setContentType("application/json;charset=UTF-8");
        objectMapper.writeValue(response.getWriter(),
                new AuthTokenResponse(accessToken, isUser, userData));
    }

    private LoginUserData extractUserData(OAuth2User oAuth2User) {
        Map<String, Object> kakaoAccount = (Map<String, Object>) oAuth2User.getAttributes().get("kakao_account");
        Map<String, Object> profile = (Map<String, Object>) kakaoAccount.get("profile");
        String nickname = (String) profile.get("nickname");
        return new LoginUserData(nickname);
    }
